...
// $Source: /usr/local/cvs/OpenSees/SRC/material/uniaxial/NLConcrete01.cpp,v $
...
OPS_NLConcrete01()
{
  ...
  if (OPS_GetIntInput(&numData, iData) != 0) {
    opserr << "WARNING invalid uniaxialMaterial NLConcrete01 tag" << endln;
    return 0;
  }
  ...
  if (numData != 4) {
    opserr << "Invalid #args, want: uniaxialMaterial NLConcrete01 " << iData[0] << "fpc? epsc0? fpcu? epscu?\n";
    return 0;
  }
  ...
  theMaterial = new NLConcrete01(iData[0], dData[0], dData[1], dData[2], dData[3]);
  
  if (theMaterial == 0) {
    opserr << "WARNING could not create uniaxialMaterial of type NLConcrete01 Material\n";
    return 0;
  }
...
int NLConcrete01::setNLStrain(double nlstrain)
{
    Tnlstrain = nlstrain;
    return 0;
}
...
int NLConcrete01::setTrialStrain (double strain, double strainRate)
{
  ...
  double m = 1.5;
  double straincheck = 0.0;
  if (Tstrain >= epsc0)
      straincheck = Tstrain;
  else
    straincheck =  m*Tnlstrain + (1-m)*Tstrain; // weighted nonlocal strain
...
int 
NLConcrete01::setTrial (double strain, double &stress, double &tangent, double strainRate)
{
	 // Reset trial history variables to last committed state
   TminStrain = CminStrain;
   TendStrain = CendStrain;
  ...
  //opserr << "NLConcrete01::setTrial() " << strain << " " << tangent << " " << strain << endln;
...
void NLConcrete01::determineTrialState (double dStrain)
{  
  TminStrain = CminStrain;
  TendStrain = CendStrain;
  TunloadSlope = CunloadSlope;
...
void NLConcrete01::envelope ()
{
  ...
  double nlepsc = m*Tnlstrain + (1-m)*Tstrain;
  double Tstresstmp = 0.0;
  ...
  if (Tstrain >= eelas)
  {
    Tstresstmp = Tstrain*Ec0;
    Ttangent  = Ec0;
  }
  else {
      Tstresstmp = std::max(eelas*Ec0 + Eh*(Tstrain - eelas),fpc);
      Ttangent = Eh;
  }
...
double NLConcrete01::getStress ()
{
   return Tstress;
}

double NLConcrete01::getStrain ()
{
   return Tstrain;
}

double NLConcrete01::getTangent ()
{
   return Ttangent;
}

int NLConcrete01::commitState ()
{
   // History variables
...
int NLConcrete01::revertToLastCommit ()
{
   // Reset trial history variables to last committed state
   TminStrain = CminStrain;
   TendStrain = CendStrain;
...
int NLConcrete01::revertToStart ()
{
	double Ec0 = 2.0*fpc/epsc0;
...
UniaxialMaterial* NLConcrete01::getCopy ()
{
   NLConcrete01* theCopy = new NLConcrete01(this->getTag(),
...
int NLConcrete01::sendSelf (int commitTag, Channel& theChannel)
{
   ...
   if (res < 0) 
      opserr << "NLConcrete01::sendSelf() - failed to send data\n";

   return res;
}

int NLConcrete01::recvSelf (int commitTag, Channel& theChannel,
                                 FEM_ObjectBroker& theBroker)
{
   ...
   if (res < 0) {
      opserr << "NLConcrete01::recvSelf() - failed to receive data\n";
      this->setTag(0);      
   }
   else {
...
void NLConcrete01::Print (OPS_Stream& s, int flag)
{
  if (flag == OPS_PRINT_PRINTMODEL_MATERIAL) {      
    s << "NLConcrete01, tag: " << this->getTag() << endln;
    s << "  fpc: " << fpc << endln;
    s << "  epsc0: " << epsc0 << endln;
    s << "  fpcu: " << fpcu << endln;
    s << "  epscu: " << epscu << endln;
...
// AddingSensitivity:BEGIN ///////////////////////////////////
int
NLConcrete01::setParameter(const char **argv, int argc, Parameter &param)
{
...
int
NLConcrete01::updateParameter(int parameterID, Information &info)
{
	switch (parameterID) {
	case 1:
...
int
NLConcrete01::activateParameter(int passedParameterID)
{
...
double
NLConcrete01::getStressSensitivity(int gradIndex, bool conditional)
{
...
int
NLConcrete01::commitSensitivity(double TstrainSensitivity, int gradIndex, int numGrads)
{
...
int
NLConcrete01::getVariable(const char *varName, Information &theInfo)
{
