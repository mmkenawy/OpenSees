...
// $Source: /usr/local/cvs/OpenSees/SRC/material/section/NLFiberSection2d.cpp,v $
                                                                        
// Written: fmk
// Created: 04/04
//
// Description: This file contains the class implementation of NLFiberSection2d.
...
ID NLFiberSection2d::code(2);

void* OPS_NLFiberSection2d()
{
    int numData = OPS_GetNumRemainingInputArgs();
    if(numData < 1) {
	opserr<<"insufficient arguments for NLFiberSection2d\n";
	return 0;
    }
    ...
    int num = 30;
    return new NLFiberSection2d(tag,num);
}

// constructors:
NLFiberSection2d::NLFiberSection2d(int tag, int num, Fiber **fibers):
  SectionForceDeformation(tag, SEC_TAG_NLFiberSection2d),
  ...
  QzBar(0.0), ABar(0.0), yBar(0.0), sectionIntegr(0), e(2), s(0), ks(0), dedh(2)
{
  if (numFibers > 0) {
    ...
    if (theMaterials == 0) {
      opserr << "NLFiberSection2d::NLFiberSection2d -- failed to allocate Material pointers";
      exit(-1);
    }
    ...
    if (matData == 0) {
      opserr << "NLFiberSection2d::NLFiberSection2d -- failed to allocate double array for material data\n";
      exit(-1);
    }
    ...
    for (int i = 0; i < numFibers; i++) {
      ...
      if (theMaterials[i] == 0) {
	opserr << "NLFiberSection2d::NLFiberSection2d -- failed to get copy of a Material\n";
	exit(-1);
      }
    }    
...
NLFiberSection2d::NLFiberSection2d(int tag, int num):
  SectionForceDeformation(tag, SEC_TAG_NLFiberSection2d),
...
NLFiberSection2d::NLFiberSection2d(int tag, int num, UniaxialMaterial **mats,
  ...
  QzBar(0.0), ABar(0.0), yBar(0.0), sectionIntegr(0), e(2), s(0), ks(0), dedh(2)
{
  ...
  if (sectionIntegr == 0) {
    opserr << "Error: NLFiberSection2d::NLFiberSection2d: could not create copy of section integration object" << endln;
    exit(-1);
  }
...
NLFiberSection2d::NLFiberSection2d():
  SectionForceDeformation(0, SEC_TAG_NLFiberSection2d),
...
int
NLFiberSection2d::addFiber(Fiber &newFiber)
{
  ...
  if(numFibers == sizeFibers) {
	  ...
	  opserr <<"NLFiberSection2d::addFiber -- failed to allocate Fiber pointers\n";
	  return -1;
      }
  ...
  if(theMaterials[numFibers] == 0) {
    opserr <<"NLFiberSection2d::addFiber -- failed to get copy of a Material\n";
    return -1;
  }
...
int
NLFiberSection2d::setTrialSectionDeformation (const Vector &deforms)
{
  int res = 0;
  ...
  double d0 = deforms(0);
  double d1 = deforms(1);
  double nld0 = NLsecdef(0);
  double nld1 = NLsecdef(1);
  ...
  for (int i = 0; i < numFibers; i++) {
    ...
    double nlstrain = nld0 - y*nld1;
    int err = 0;
    err += theMat->setNLStrain(nlstrain);
    if (err != 0)
    	opserr <<"FiberSection2d::setSectionTrialDeformation -- failed to set NLstrain\n";
...
const Vector&
NLFiberSection2d::getSectionDeformation(void)
{
  return e;
}

const Matrix&
NLFiberSection2d::getInitialTangent(void)
{
...
const Matrix&
NLFiberSection2d::getSectionTangent(void)
{
  return *ks;
}

const Vector&
NLFiberSection2d::getStressResultant(void)
{
  return *s;
}

SectionForceDeformation*
NLFiberSection2d::getCopy(void)
{
  NLFiberSection2d *theCopy = new NLFiberSection2d ();
  ...
  if (numFibers != 0) {
    ...
    if (theCopy->theMaterials == 0) {
      opserr <<"NLFiberSection2d::getCopy -- failed to allocate Material pointers\n";
      exit(-1);
    }
    ...
    if (theCopy->matData == 0) {
      opserr << "NLFiberSection2d::getCopy -- failed to allocate double array for material data\n";
      exit(-1);
    }
	...
	opserr <<"NLFiberSection2d::getCopy -- failed to get copy of a Material";
	exit(-1);
      }
    }  
  }
...
const ID&
NLFiberSection2d::getType ()
{
  return code;
}

int
NLFiberSection2d::getOrder () const
{
  return 2;
}

int
NLFiberSection2d::commitState(void)
{
  int err = 0;
...
int
NLFiberSection2d::revertToLastCommit(void)
{
  int err = 0;
...
int
NLFiberSection2d::revertToStart(void)
{
...
int
NLFiberSection2d::sendSelf(int commitTag, Channel &theChannel)
{
  int res = 0;
  ...
  if (res < 0) {
    opserr <<  "NLFiberSection2d::sendSelf - failed to send ID data\n";
    return res;
  }    
  ...
  if (numFibers != 0) {
    ...
    if (res < 0) {
      opserr <<  "NLFiberSection2d::sendSelf - failed to send material data\n";
      return res;
    }    
...
int
NLFiberSection2d::recvSelf(int commitTag, Channel &theChannel,
			 FEM_ObjectBroker &theBroker)
{
  ...
  if (res < 0) {
    opserr <<  "NLFiberSection2d::recvSelf - failed to recv ID data\n";
    return res;
  }    
  this->setTag(data(0));
  ...
  if (data(1) != 0) {
    ...
    if (res < 0) {
      opserr <<  "NLFiberSection2d::recvSelf - failed to recv material data\n";
      return res;
    }    
	...
	if (theMaterials == 0) {
	  opserr <<"NLFiberSection2d::recvSelf -- failed to allocate Material pointers\n";
	  exit(-1);
	}
	...
	if (matData == 0) {
	  opserr <<"NLFiberSection2d::recvSelf  -- failed to allocate double array for material data\n";
	  exit(-1);
	}
      }
    }
...
void
NLFiberSection2d::Print(OPS_Stream &s, int flag)
{
  if (flag == OPS_PRINT_PRINTMODEL_SECTION || flag == OPS_PRINT_PRINTMODEL_MATERIAL) {
    s << "\nNLFiberSection2d, tag: " << this->getTag() << endln;
    s << "\tSection code: " << code;
...
Response*
NLFiberSection2d::setResponse(const char **argv, int argc,
			    OPS_Stream &output)
{
...
int 
NLFiberSection2d::getResponse(int responseID, Information &sectInfo)
{
  if (responseID == 5) {
...
int
NLFiberSection2d::setParameter(const char **argv, int argc, Parameter &param)
{
  if (argc < 1)
    return -1;
...
const Vector &
NLFiberSection2d::getSectionDeformationSensitivity(int gradIndex)
{
...
const Vector &
NLFiberSection2d::getStressResultantSensitivity(int gradIndex, bool conditional)
{
  static Vector ds(2);
...
const Matrix &
NLFiberSection2d::getInitialTangentSensitivity(int gradIndex)
{
...
int
NLFiberSection2d::commitSensitivity(const Vector& defSens,
...
//by SAJalali
double NLFiberSection2d::getEnergy() const
{
	static double fiberArea[10000];
...
void
NLFiberSection2d::setNLsectiondeformation(const Vector &nldef)
{
    NLsecdef = nldef;
}
