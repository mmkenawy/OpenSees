...
// Created: August 2020
//
// Description: This file contains the class definition for NLDispBeamColumn2d.
...
Matrix NLDispBeamColumn2d::K(6,6);
Vector NLDispBeamColumn2d::P(6);
double NLDispBeamColumn2d::workArea[100];
...
void* OPS_NLDispBeamColumn2d()
{
	...
	if(strcmp(type, "-nllength") == 0) {
		if(OPS_GetDoubleInput(&numData,&nllength) < 0) {
			opserr<<"WARNING: invalid nonlocal length parameter\n";
			return 0;
		}
		}
	}
    ...
    Element *theEle =  new NLDispBeamColumn2d(iData[0],iData[1],iData[2],secTags.Size(),sections,
					    *bi,*theTransf,iData[5],nllength,mass,cmass);
    delete [] sections;
    return theEle;
}

void* OPS_NLDispBeamColumn2d(const ID &info)
{
    // data
    int iData[6];
    int numData;
...
int OPS_NLDispBeamColumn2d(Domain& theDomain, const ID& elenodes, ID& eletags)
{
	...
	theEle = new NLDispBeamColumn2d(--currTag,elenodes(2*i),elenodes(2*i+1),secTags.Size(),
				      sections,*bi,*theTransf,iData[2],nllength,mass,cmass);
	if (theEle == 0) {
...
NLDispBeamColumn2d::NLDispBeamColumn2d(int tag, int nd1, int nd2,
				   int numSec, SectionForceDeformation **s,
				   BeamIntegration& bi,
				   CrdTransf &coordTransf, int memID, double nllength, double r, int cm)
:Element (tag, ELE_TAG_NLDispBeamColumn2d),
 numSections(numSec), theSections(0), crdTransf(0), beamInt(0),
  connectedExternalNodes(2),
  Q(6), q(3), memberID(memID), nllen(nllength), rho(r), cMass(cm), parameterID(0)
{
  // Allocate arrays of pointers to SectionForceDeformations
  theSections = new SectionForceDeformation *[numSections];
    
  if (theSections == 0) {
    opserr << "NLDispBeamColumn2d::NLDispBeamColumn2d - failed to allocate section model pointer\n";
    exit(-1);
  }
  ...
  for (int i = 0; i < numSections; i++) {
    ...
    // Check allocation
    if (theSections[i] == 0) {
      opserr << "NLDispBeamColumn2d::NLDispBeamColumn2d -- failed to get a copy of section model\n";
      exit(-1);
    }
  }
  ...
  if (beamInt == 0) {
    opserr << "NLDispBeamColumn2d::NLDispBeamColumn2d - failed to copy beam integration\n";
    exit(-1);
  }
  ...
  if (crdTransf == 0) {
    opserr << "NLDispBeamColumn2d::NLDispBeamColumn2d - failed to copy coordinate transformation\n";
    exit(-1);
  }
...
NLDispBeamColumn2d::NLDispBeamColumn2d()
:Element (0, ELE_TAG_NLDispBeamColumn2d),
 numSections(0), theSections(0), crdTransf(0), beamInt(0),
 connectedExternalNodes(2),
  Q(6), q(3), memberID(0), nllen(0.0), rho(0.0), cMass(0), parameterID(0)
{
    q0[0] = 0.0;
    q0[1] = 0.0;
    q0[2] = 0.0;
...
int
NLDispBeamColumn2d::getNumExternalNodes() const
{
    return 2;
}

const ID&
NLDispBeamColumn2d::getExternalNodes()
{
    return connectedExternalNodes;
}

Node **
NLDispBeamColumn2d::getNodePtrs()
{
    return theNodes;
}

int
NLDispBeamColumn2d::getNumDOF()
{
    return 6;
}

void
NLDispBeamColumn2d::setDomain(Domain *theDomain)
{
    ...
    if (theNodes[0] == 0 || theNodes[1] == 0) {
      opserr << "WARNING NLDispBeamColumn2d (tag: %d), node not found in domain" << this->getTag() << endln;;
      return;
    }
	...
	//opserr << "FATAL ERROR NLDispBeamColumn2d (tag: %d), has differing number of DOFs at its nodes",
	//	this->getTag());
...
int
NLDispBeamColumn2d::commitState()
{
    int retVal = 0;

    // call element commitState to do any base class stuff
    if ((retVal = this->Element::commitState()) != 0) {
      opserr << "NLDispBeamColumn2d::commitState () - failed in base class";
    }    
...
int
NLDispBeamColumn2d::revertToLastCommit()
{
    int retVal = 0;
...
int
NLDispBeamColumn2d::revertToStart()
{
    int retVal = 0;
...
int
NLDispBeamColumn2d::update(void)
{
  int err = 0;

  this->computeNLstrain();
  ...
  for (int i = 0; i < numSections; i++) {
	...
	if (flag == true) {
		theSections[i]->setNLsectiondeformation(e);
	}
  ...
  if (err != 0) {
    opserr << "NLDispBeamColumn2d::update() - failed setTrialSectionDeformations()\n";
    return err;
  }
...
void
NLDispBeamColumn2d::computeNLstrain()
{
  ...
  //get the number of elements in the domain - MK
    ...
    //int numFibers = theSections[0]->getNumFibers();
    int order = theSections[0]->getOrder();

    static Matrix defMat(numEle*numSections,order);
    defMat.Zero();
    static Matrix NLdefMat(numEle*numSections,order);
    NLdefMat.Zero();

  double AccumLen = 0.0;
  for (int k = 0; k < numEle; k++) {

  // get pointers to other elements' sections and crd transformations
  Element *aptr = theDomain->getElement(k+1);
  NLDispBeamColumn2d *bptr = (NLDispBeamColumn2d*)aptr;
  SectionForceDeformation** Sections = bptr->getSecPtr();
  CrdTransf* ncrdTransf = bptr->getCrdTransf();


  // Update the transformation
  ncrdTransf->update();
  //   Get basic deformations
  const Vector &v = ncrdTransf->getBasicTrialDisp();

  double Len = ncrdTransf->getInitialLength();
  double oneOverL = 1.0/Len;

  //const Matrix &pts = quadRule.getIntegrPointCoords(numSections);
  double xi[maxNumSections]={};
  beamInt->getSectionLocations(numSections, Len, xi);
 ...
 // populate a matrix with the section deformations for each section
   for (int j = 0; j < order; j++) {
       defMat((i+(k*numSections)),j) = e(j);
       double defmatcheck = defMat((i+(k*numSections)),j);
       double useless2 = defmatcheck;
   }
  }
  }

    // calculate the coordinate of each IP
    static Vector coord(numEle*numSections);
    static Vector gwt(numEle*numSections);
    coord.Zero();
    for (int k = 0; k < numEle; k++) {
            ...
            beamInt->getSectionWeights(numSections, Len, wt);

        for (int i = 0; i < numSections; i++) {
            coord(i+(k*numSections)) = xi[i]*Len + AccumLen;
            double coordcheck = coord(i+(k*numSections));
            gwt(i+(k*numSections)) = wt[i]*Len;
            double gwtcheck = gwt(i+(k*numSections));
        }
              AccumLen += Len;
    }

// get tag of element you're in
    int tag = this->getTag();

// calculate the weights in the weight function
     double R = nllen;
    const double PI = 3.141592653589793;
    //const double wtdenom = 1/(R*sqrt(2*PI));
    //const double over2R2 = 1/(2*pow(R,2));
    double overR2 = 1/pow(R,2);
    static Vector w(numEle*numSections);
    ...
    for (int m = 0; m < numEle*numSections; m++) {
    double tempr = fabs(coord(i+((tag-1)*numSections))-coord(m));
    w(m) = gwt(m)*pow(std::max((1 - pow(tempr,2)*overR2),0.0),2);
    //w(m) = gwt(m)*exp(-1*pow(tempr/Lc,2))/(Lc*sqrt(PI));
    //w(m) = gwt(m)*exp(-1*pow(tempr,2)*over2R2)*wtdenom;
    double wtcheck = w(m);
    wsum += w(m);
    }
    ...
    Vector NLe(order);
    NLe.Zero();
// now calculate nonlocal section deformations

    for (int j = 0; j < order; j++) {
        for (int m = 0; m < numEle*numSections; m++) {
        NLe(j) += (w(m)*defMat(m,j))/wsum;
    }
    }
    double defcheck1 = NLe(0);
    double defcheck2 = NLe(1);
          theSections[i]->setNLsectiondeformation(NLe);
     }
}
}

SectionForceDeformation**
NLDispBeamColumn2d::getSecPtr()
{
    return theSections;
}

CrdTransf*
NLDispBeamColumn2d::getCrdTransf()
{
    return crdTransf;
}

void
NLDispBeamColumn2d::getBasicStiff(Matrix &kb, int initial)
{
  // Zero for integral
  kb.Zero();
...
const Matrix&
NLDispBeamColumn2d::getTangentStiff()
{
...
const Matrix&
NLDispBeamColumn2d::getInitialBasicStiff()
{
  static Matrix kb(3,3);
...
const Matrix&
NLDispBeamColumn2d::getInitialStiff()
{
...
void
NLDispBeamColumn2d::zeroLoad(void)
{
  Q.Zero();
...
int 
NLDispBeamColumn2d::addLoad(ElementalLoad *theLoad, double loadFactor)
{
  int type;
  ...
  if (type == LOAD_TAG_Beam2dUniformLoad) {
    ...
    opserr << "NLDispBeamColumn2d::NLDispBeamColumn2d -- load type unknown for element with tag: "
	   << this->getTag() << "NLDispBeamColumn2d::addLoad()\n";
...
int 
NLDispBeamColumn2d::addInertiaLoadToUnbalance(const Vector &accel)
{
  ...
  if (3 != Raccel1.Size() || 3 != Raccel2.Size()) {
    opserr << "NLDispBeamColumn2d::addInertiaLoadToUnbalance matrix and vector sizes are incompatible\n";
    return -1;
  }
...
const Vector&
NLDispBeamColumn2d::getResistingForce()
{
  double L = crdTransf->getInitialLength();
...
const Vector&
NLDispBeamColumn2d::getResistingForceIncInertia()
{
...
int
NLDispBeamColumn2d::sendSelf(int commitTag, Channel &theChannel)
{
  ...
  if (theChannel.sendVector(dbTag, commitTag, data) < 0) {
    opserr << "NLDispBeamColumn2d::sendSelf() - failed to send data Vector\n";
     return -1;
  }
  
  // send the coordinate transformation
  if (crdTransf->sendSelf(commitTag, theChannel) < 0) {
     opserr << "NLDispBeamColumn2d::sendSelf() - failed to send crdTranf\n";
     return -1;
  }      

  // send the beam integration
  if (beamInt->sendSelf(commitTag, theChannel) < 0) {
    opserr << "NLDispBeamColumn2d::sendSelf() - failed to send beamInt\n";
    return -1;
  }      
  ...
  if (theChannel.sendID(dbTag, commitTag, idSections) < 0)  {
    opserr << "NLDispBeamColumn2d::sendSelf() - failed to send ID data\n";
    return -1;
  }    
  ...
  for (j = 0; j<numSections; j++) {
    if (theSections[j]->sendSelf(commitTag, theChannel) < 0) {
      opserr << "NLDispBeamColumn2d::sendSelf() - section " <<
...
int
NLDispBeamColumn2d::recvSelf(int commitTag, Channel &theChannel,
			   FEM_ObjectBroker &theBroker)
{
  ...
  if (theChannel.recvVector(dbTag, commitTag, data) < 0)  {
    opserr << "NLDispBeamColumn2d::recvSelf() - failed to recv data Vector\n";
    return -1;
  }
  ...
  if (crdTransf == 0 || crdTransf->getClassTag() != crdTransfClassTag) {
      ...
      if (crdTransf == 0) {
	opserr << "NLDispBeamColumn2d::recvSelf() - failed to obtain a CrdTrans object with classTag " <<
  ...
  if (crdTransf->recvSelf(commitTag, theChannel, theBroker) < 0) {
    opserr << "NLDispBeamColumn2d::sendSelf() - failed to recv crdTranf\n";
    return -3;
  }      
  ...
  if (beamInt == 0 || beamInt->getClassTag() != beamIntClassTag) {
      ...
      if (beamInt == 0) {
	opserr << "NLDispBeamColumn2d::recvSelf() - failed to obtain the beam integration object with classTag" <<
  ...
  {
     opserr << "NLDispBeamColumn2d::sendSelf() - failed to recv beam integration\n";
     return -3;
  }      
  ...
  if (theChannel.recvID(dbTag, commitTag, idSections) < 0)  {
    opserr << "NLDispBeamColumn2d::recvSelf() - failed to recv ID data\n";
    return -1;
  }    
  ...
  if (numSections != nSect) {
    ...
    if (theSections == 0) {
opserr << "NLDispBeamColumn2d::recvSelf() - out of memory creating sections array of size " <<
  nSect << endln;
      return -1;
    }    
    ...
    for (i=0; i<numSections; i++) {
      ...
      if (theSections[i] == 0) {
	opserr << "NLDispBeamColumn2d::recvSelf() - Broker could not create Section of class type " <<
	  sectClassTag << endln;
	exit(-1);
      }
      theSections[i]->setDbTag(sectDbTag);
      if (theSections[i]->recvSelf(commitTag, theChannel, theBroker) < 0) {
	opserr << "NLDispBeamColumn2d::recvSelf() - section " << i << " failed to recv itself\n";
	return -1;
      }     
    }
...
void
NLDispBeamColumn2d::Print(OPS_Stream &s, int flag)
{
  if (flag == OPS_PRINT_CURRENTSTATE) {
    s << "\nNLDispBeamColumn2d, element id:  " << this->getTag() << endln;
...
int
NLDispBeamColumn2d::displaySelf(Renderer &theViewer, int displayMode, float fact, const char **displayModes, int numModes)
{
  static Vector v1(3);
  static Vector v2(3);
...
Response*
NLDispBeamColumn2d::setResponse(const char **argv, int argc,
			      OPS_Stream &output)
{
  Response *theResponse = 0;

  output.tag("ElementOutput");
  output.attr("eleType","NLDispBeamColumn2d");
...
int 
NLDispBeamColumn2d::getResponse(int responseID, Information &eleInfo)
{
  double V;
  double L = crdTransf->getInitialLength();
...
int 
NLDispBeamColumn2d::getResponseSensitivity(int responseID, int gradNumber,
					 Information &eleInfo)
{
  // Basic deformation sensitivity
  if (responseID == 3) {  
...
int
NLDispBeamColumn2d::setParameter(const char **argv, int argc, Parameter &param)
{
  if (argc < 1)
    return -1;
...
int
NLDispBeamColumn2d::updateParameter (int parameterID, Information &info)
{
  if (parameterID == 1) {
    rho = info.theDouble;
    return 0;
...
int
NLDispBeamColumn2d::activateParameter(int passedParameterID)
{
...
const Matrix &
NLDispBeamColumn2d::getInitialStiffSensitivity(int gradNumber)
{
  static Matrix kb(3,3);
...
const Matrix &
NLDispBeamColumn2d::getMassSensitivity(int gradNumber)
{
  K.Zero();
...
const Vector &
NLDispBeamColumn2d::getResistingForceSensitivity(int gradNumber)
{
  double L = crdTransf->getInitialLength();
  double oneOverL = 1.0/L;
...
// NEW METHOD
int
NLDispBeamColumn2d::commitSensitivity(int gradNumber, int numGrads)
{
