...
#ifndef NLDispBeamColumn2d_h
#define NLDispBeamColumn2d_h
...
class NLDispBeamColumn2d : public Element
{
  public:
    NLDispBeamColumn2d(int tag, int nd1, int nd2,
		     int numSections, SectionForceDeformation **s,
		     BeamIntegration &bi, CrdTransf &coordTransf,
			 int memberID, double nllen = 0.0,
		     double rho = 0.0, int cMass = 0);
    NLDispBeamColumn2d();
    ~NLDispBeamColumn2d();

    const char *getClassType(void) const {return "NLDispBeamColumn2d";};
    ...
    int update(void);
    void computeNLstrain(void);
    SectionForceDeformation** getSecPtr(void);
    CrdTransf* getCrdTransf(void);
